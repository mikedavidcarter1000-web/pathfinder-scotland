...
export default function ParentWelcomePage() {
  return (
    <div className="pf-container pt-8 pb-12" style={{ maxWidth: '820px' }}>
      ...
      {/* Google Translate widget (top, as specified) */}
      <GoogleTranslate />
      ...
      {/* English welcome content -- source of truth */}
      ...
      {/* Table of language contents -- jump links */}
      <nav
      ...
      >
        <h2 style={{ fontSize: '1rem', marginBottom: '10px' }}>
          Jump to your language / Leum gu do chànan
        </h2>
        <div
          style={{
            display: 'flex',
        ...
        >
          {BLOCKS.map((b) => (
            <a
              key={b.bcp47 + b.languageName}
              href={`#lang-${b.bcp47}-${slugify(b.languageName)}`}
              style={{ color: 'var(--pf-blue-700)' }}
            >
              {b.languageNameNative}
            </a>
          ))}
        </div>
      </nav>

      {/* Statutory + indigenous languages divider */}
      <SectionHeading>Languages of Scotland</SectionHeading>
      {BLOCKS.slice(0, 2).map((b) => (
        <LanguageSection key={b.bcp47} block={b} />
      ))}

      <SectionHeading>European community languages</SectionHeading>
      {BLOCKS.slice(2, 16).map((b) => (
        <LanguageSection key={b.bcp47 + b.languageName} block={b} />
      ))}

      <SectionHeading>South Asian community languages</SectionHeading>
      {BLOCKS.slice(16, 20).map((b) => (
        <LanguageSection key={b.bcp47 + b.languageName} block={b} />
      ))}

      <SectionHeading>East and Southeast Asian languages</SectionHeading>
      {BLOCKS.slice(20, 22).map((b) => (
        <LanguageSection key={b.bcp47 + b.languageName} block={b} />
      ))}

      <SectionHeading>Middle Eastern and African languages</SectionHeading>
      {BLOCKS.slice(22).map((b) => (
        <LanguageSection key={b.bcp47 + b.languageName} block={b} />
      ))}
...
function LanguageSection({ block }: { block: LanguageBlock }) {
  const anchor = `lang-${block.bcp47}-${slugify(block.languageName)}`
  return (
    <section
      id={anchor}
      style={{
    ...
    >
      ...
      >
        ...
        <span style={{ color: 'var(--pf-grey-500)' }}>/</span>

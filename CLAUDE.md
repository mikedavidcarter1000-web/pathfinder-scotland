...
- **Migration application: use the wrapper script, not bare `supabase db push`.** After creating a migration file with `npx supabase migration new <name>`, apply it via `scripts/apply-migration.sh <filename>` (bash / Git Bash) or `scripts/apply-migration.ps1 <filename>` (PowerShell). The script runs `supabase db push`, queries the remote `supabase_migrations.schema_migrations` for the actual applied version, and auto-renames the local file if the timestamps differ. This eliminates the recurring manual rename step (observed across 8 consecutive sessions, ~10 min total rework each session).
